...
selectedChannels   = newArray(3,6);  // with multiple values passed to new array, this directly stores the values in the array ie the array size is automatically adjusted to the number of values
selectedZ          = newArray(0);    // here we pass 0 to the size argument of the array, in effect creating an empty array. As a result, all available Z-positions will be taken
